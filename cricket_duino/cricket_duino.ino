...
// random sleep time minimum and maximum (24 hours to 31 days) in minutes
const unsigned int sleepTimeMIN = 1440;
const unsigned int sleepTimeMAX = 44640;
...
void chirpAndSleep() {
  ...
  if (repeat > 0) { // If we're making more than one noise, don't sleep
    ...
    if (demoMode == true) { // If we're in demo mode, don't sleep.
      ...
      int sleepTime = random(sleepTimeMIN, sleepTimeMAX); // Get random time from sleepTimes list
      // Nighty night
...
void chirp() {
  int freq = random(3600, 8200); // random frequency in this range is picked for this chirp
  float volume = random(50, 100+1) / 100.00; // random volume between 50-100% is chosen for this chirp
